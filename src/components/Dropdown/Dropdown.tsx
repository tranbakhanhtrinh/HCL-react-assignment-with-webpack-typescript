...
const Dropdown = () => {
    ...
    const [searchArray, setSearchArray] = useState<string[]>(isSearchArrayInLocalStorage ? getItem("search") : []);
    ...
    return (
        <Wrapper ref={ref}>
            <Input type="text" id="searchProduct" label="Search" onChange={onHandleSearchProduct} onFocus={onHandleFocusSearchProduct} onBlur={onHandleBlurSearchProduct} onKeyPress={onHandlePressEnter} value={searchText} />
            {((focus && !searchText && historyItemsLength > 0) || (focus && searchText && historyItemsLength > 0) || (!focus && showSearch) || (!focus && showSearch && !searchText)) && (
                <HistorySearchWrapper>
                    {searchArray.map((sa) => (
                        <HistorySearchItemWrapper key={`${id}-${sa}`} onClick={() => onHandleChooseItem(sa)} onMouseOver={onMouseOver}>
                            <div>
                                <i className="fa-regular fa-clock me-2"></i>
                                {sa}
                            </div>
                            <i className="fa-solid fa-xmark" onClick={(e) => onDeleteHistoryItem(e, sa)}></i>
